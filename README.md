...
翻訳文に加えてバックトランスレーションも表示し、「相手にどう伝わるか」を確認しながら会話できます。

このプロジェクトは、AI を使ったユーザー体験に加えて、テスト、レビュー、デプロイ、承認付き本番反映、VPS 運用まで含むポートフォリオです。実際に使える音声通訳アプリを題材に、機能実装と運用品質の両方を設計・構築しています。
...
テスト方針と coverage は [docs/testing.md](docs/testing.md) を参照してください。CI/CD の詳細は [docs/ci-cd.md](docs/ci-cd.md) にまとめています。
...
**merge-ready の制限:** Codex がコメントで OK 文言を返した場合は workflow が `merge-ready` を自動付与します。Codex が 👍 リアクションのみで完了した場合は workflow の自動検知対象外です。その場合は内容を確認のうえ `merge-ready` ラベルを手動付与してください。
...
GoTalk では、公開環境の死活監視と障害通知に Uptime Kuma を使用しています。
